            ...
            <div class="card shadow mb-4">
                <div class="card-body">
                    <form action="{{ route('diagnosis.update', ['diagnosi' => $data->id]) }}" method="post">
                        ...
                        <div class="form-group">
                            <label for="tooths">Tooths</label>
                            <div id="hidden-tooth-inputs"></div>
                            <label for="selectAllTeeth">
                                <input type="checkbox" id="selectAllTeeth"> Select All Teeth
                            </label>
                            @error('tooths[]')
                                <p style="color: red">* {{ $message }}</p>
                            @enderror
                            <div class="tooth-chart">
                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <g id="Spots">
                                        <polygon id="Tooth32" fill="#0000" data-key="32"
                                        ...
                                        <path id="Tooth28" fill="#FFFFFF" data-key="28"
                                        ...
                                        <path id="Tooth24" fill="#FFFFFF" data-key="24"
                                        ...
                                        <path id="Tooth19" fill="#FFFFFF" data-key="19"
                                        ...
                                        <path id="Tooth7" fill="#FFFFFF" data-key="7"
...
@section('script')
    <script>
        $(document).ready(function() {
            // Pre-fill selected teeth from the server (array from Laravel)
            let selectedTeeth = @json(old('tooths', $data->tooths)); // Pre-filled teeth from the database or old input

            // Mark the pre-selected teeth as selected
            selectedTeeth.forEach(function(tooth) {
                $(`polygon[data-key="${tooth}"], path[data-key="${tooth}"]`).addClass('selected');
            });

            // Update the hidden input fields dynamically for pre-selected teeth
            updateHiddenInputs();
            ...
            $(document).on("click", "polygon, path", function() {
                ...
                if (toothNumber !== undefined) {
                    if ($(this).hasClass("selected")) {
                        // If the tooth is already selected, remove it from the list
                    ...
                    // Uncheck the "Select All" checkbox if not all teeth are selected
                    if (selectedTeeth.length !== $('polygon[data-key], path[data-key]').length) {
                        $('#selectAllTeeth').prop('checked', false);
                    }
                }
            });

            // Event listener for the "Select All" checkbox
            $('#selectAllTeeth').on('change', function() {
                if ($(this).is(':checked')) {
                    // Select all teeth with valid data-key attributes
                    $('polygon[data-key], path[data-key]').each(function() {
                        const toothNumber = $(this).data("key");
                        if (!$(this).hasClass('selected')) {
            ...
            // Function to update the hidden inputs with the selected teeth
            function updateHiddenInputs() {
                // Clear any existing hidden inputs
                $('#hidden-tooth-inputs').empty();

                // Create hidden input fields for each selected tooth
                selectedTeeth.forEach(tooth => {
                    $('#hidden-tooth-inputs').append(
                        `<input type="hidden" name="tooths[]" value="${tooth}">`
                    );
                });
            }
        });
